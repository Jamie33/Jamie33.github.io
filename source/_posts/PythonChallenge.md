...
线索：打开源代码，第一段注释中有个zip，第二段注释和本谜题无关。把zip替代url中的html，获取到一个zip文件
...
```
    ...
    <ipython-input-4-6122cbca773d> in <module>()
         21             with open(os.path.join(os.getcwd(),'PC6zipfiles',a),'r') as text:
         22                 nothing = text.read()
    ---> 23                 if re.search(r'(\d){2,5}',nothing).group():
         24                     hint = re.search(r'(\d){2,5}',nothing).group()
         25                     comments.append(f.getinfo('%s.txt'%hint).comment)
